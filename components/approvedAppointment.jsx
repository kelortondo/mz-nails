...
function ApprovedAppointment({req, handleRerender}) {
  ...
  return(
    <div className={styles.aptApproved}>
      ...
      <p style={{marginLeft: '1rem'}}>Location: {req.location[0].toUpperCase() + req.location.slice(1)} || Date/Time: {new Date(req.aptDate).toLocaleString('en-AR', {timeZone: 'America/Argentina/Buenos_Aires', hour12: false, dateStyle: 'medium', timeStyle: 'short'})} ({req.duration} hr) || Service: {req.service}{req.pedicure ? ', pedicure' : ''}{req.manicure ? ', manicure' : ''}</p>
      <div>
